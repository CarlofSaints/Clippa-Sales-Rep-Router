 ...
 * every child of "/api/auth" public too, including
 * /api/auth/change-password, a route that took a userId from the request body,
 * set that user's password and handed back a signed session cookie. One word,
 * total compromise. If you add an entry here, add the one route that is public,
 * not the folder it lives in.
 *
 * The password reset pair is public by necessity: the person who needs them is
 * the person who cannot sign in. Each protects itself instead.
