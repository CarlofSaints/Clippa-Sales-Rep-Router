 ...
 * Rep home addresses are often informal — "Stand no 644 moletjie ga semenya
 * polokwane" — and Google answers those by falling back to the suburb, the
 * town, or the province centroid, with no error. Saving that silently would
 * anchor a rep's entire week on a point tens of kilometres from where they
 * live, and it would look perfectly plausible on the map. Callers must decide
 * what confidence is good enough; this function does not decide for them.
 *
