 ...
 *
 * Three callers, with deliberately DIFFERENT password behaviour:
 *
 ...
 *
 *   POST /api/reps/create-account a rep's first login, minted fresh. Uses the
 *                                 "rep" audience, which says set your home
 *                                 address rather than describing an app they
 *                                 cannot otherwise reach.
 *
 * Nothing here throws. A mail failure must not roll back a user that was created
 * successfully, so every path returns a result the caller can put on screen.
 */
...
export function resolveAppUrl(): string {
  return process.env.NEXT_PUBLIC_APP_URL || "https://clippa-sales-rep-router.vercel.app";
}
...
export function buildWelcomeEmail(input: WelcomeEmailInput): {
  ...
  const logoUrl = `${appUrl}/clippa-logo.jpg`;
  ...
  // middleware redirects them there anyway; landing on it directly just saves a
  // hop and matches what the instructions below tell them to open.
  const signInUrl = isRep ? `${appUrl}/account` : appUrl;

  const headline = isRep ? "Set where your day starts" : "Welcome to the Rep Router";
  const subhead = isRep ? "It takes about a minute" : "Your account is ready";
  const buttonLabel = isRep ? "Sign in and set your home address" : "Sign in to Clippa Rep Router";

  const intro = isRep
    ? `An account has been created for you on <strong>Clippa Rep Router</strong>, the system that plans your call cycle. ` +
    ...
    : `An account has been created for you on <strong>Clippa Rep Router</strong>, where call cycles, rep journeys and ` +
...
<html>
  <head>
    ...
    <title>${isRep ? "Set where your day starts" : "Welcome to Clippa Rep Router"}</title>
  </head>
  <body style="margin:0;padding:0;background:${BRAND.tint};">
    ...
    <div style="display:none;max-height:0;overflow:hidden;opacity:0;">${
      isRep
        ...
        : "Your Clippa Rep Router sign-in details are inside."
    }</div>
...
${row("Temporary password", password, true)}
            ...
            <tr>
              <td align="center" style="padding:12px 32px 0 32px;font-family:Helvetica,Arial,sans-serif;font-size:12px;color:${BRAND.grey};">
                ...
                <a href="${signInUrl}" style="color:${BRAND.redDark};text-decoration:none;">${signInUrl}</a>
              </td>
            </tr>
${repInstructions}
            ...
            <tr>
              <td style="padding:28px 32px 0 32px;font-family:Helvetica,Arial,sans-serif;font-size:15px;line-height:1.6;color:${BRAND.dark};">
                <p style="margin:0;">Regards,<br><strong>The Clippa Sales Team</strong></p>
              </td>
            </tr>
  ...
  const text = [
    ...
    ``,
    isRep
      ? `An account has been created for you on Clippa Rep Router, the system that plans your call cycle. Please sign in and tell us where you live: your route is planned outwards from your home, so the closer it is to your front door, the less driving you do.`
      : `An account has been created for you on Clippa Rep Router.`,
    ``,
  ...
  return {
    subject: isRep
      ? "Your Clippa Rep Router login, please set your home address"
      : "Welcome to Clippa Rep Router",
    html,
    text,
  };
}
 ...
 *
 * It carries a LINK and never a password. It cannot carry a password: they are
 * stored as bcrypt hashes and a hash is one way, so there is nothing to send
...
<html>
  <head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <title>Reset your Clippa Rep Router password</title>
  </head>
  <body style="margin:0;padding:0;background:${BRAND.tint};">
        ...
        <td align="center" style="padding:32px 12px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:560px;background:#ffffff;border:1px solid #f0e2e2;border-radius:14px;overflow:hidden;">
            ...
            <tr>
              <td style="padding:20px 32px 0 32px;font-family:Helvetica,Arial,sans-serif;font-size:15px;line-height:1.6;color:${BRAND.dark};">
                <p style="margin:0 0 14px 0;">Hi ${name},</p>
                <p style="margin:0;">Somebody asked to reset the password on your <strong>Clippa Rep Router</strong> account. Tap the button to choose a new one.</p>
              </td>
            </tr>
            ...
            <tr>
              <td align="center" style="padding:12px 32px 0 32px;font-family:Helvetica,Arial,sans-serif;font-size:12px;color:${BRAND.grey};">
                or paste this into your browser:<br>
                <a href="${url}" style="color:${BRAND.redDark};text-decoration:none;word-break:break-all;">${url}</a>
              </td>
            </tr>
  ...
  const text = [
    `Hi ${input.name},`,
    ``,
    `Somebody asked to reset the password on your Clippa Rep Router account.`,
