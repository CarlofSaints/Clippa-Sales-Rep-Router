 ...
 *
 * Reps exist in this app for one reason — to maintain their OWN profile, chiefly
 ...
 * matrix. `getRolePermissions()` reads a saved blob and only backfills roles that
 * are MISSING from it, so editing the `rep` defaults in code changes nothing on a
 * deployment that has already saved its roles — and this one has. A gate that can
...
const REP_ALLOWED = [
  ...
  "/login",
  // The guide explains the whole system, including the one page a rep uses.
  // It reads no data of its own, so there is nothing here to scope, and a rep
  // being unable to read the instructions for their own screen is silly.
  "/guide",
];
