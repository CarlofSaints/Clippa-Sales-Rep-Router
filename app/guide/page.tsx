...
const TONE = {
  ...
  danger: { border: "border-red-200", bg: "bg-red-50", head: "text-red-900", body: "text-red-800", icon: "text-clippa-red" },
} as const;
        ...
        <ol className="mb-5 space-y-3">
          {block.items.map((item, i) => (
            <li key={i} className="flex gap-3 break-inside-avoid">
              <span className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-clippa-red text-[11px] font-bold text-white">
                {i + 1}
              </span>
        ...
        <ul className="mb-5 space-y-1.5">
          {block.items.map((t, i) => (
            <li key={i} className="flex gap-2 text-sm text-gray-700">
              <span className="mt-1.5 h-1 w-1 flex-shrink-0 rounded-full bg-clippa-red" />
              <span>{t}</span>
            </li>
          ))}
        </ul>
  ...
  return (
    <section id={section.id} className="guide-section scroll-mt-6">
      <div className="mb-4 flex items-start gap-3 border-b border-gray-200 pb-3">
        <span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-clippa-red/10">
          <svg className="h-5 w-5 text-clippa-red" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.7}>
            <path strokeLinecap="round" strokeLinejoin="round" d={section.icon} />
          </svg>
        </span>
        <div className="min-w-0">
          <p className="text-[11px] font-bold uppercase tracking-wider text-clippa-red">Section {section.number}</p>
    ...
    <div className="guide-root mx-auto max-w-4xl p-6">
      ...
      <header className="guide-cover mb-8">
        <div className="mb-5 flex items-center justify-between gap-4">
          <Image src="/clippa-logo.jpg" alt="Clippa Sales" width={150} height={51} className="h-11 w-auto" priority />
          <button
            onClick={() => window.print()}
            className="guide-hide-print inline-flex items-center gap-2 rounded-lg bg-clippa-red px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700"
          >
        ...
        <h1 className="text-2xl font-bold text-gray-900">Rep Router: the complete guide</h1>
        <p className="mt-1 text-sm text-gray-600">
          How to load your data, build a call cycle, and keep it current. Written for the person who runs it, start to
          finish.
        </p>
        <p className="mt-3 text-xs text-gray-400">
          Version {GUIDE_VERSION} · Clippa Sales Rep Router · Powered by OuterJoin
        </p>
      ...
      <nav className="guide-toc mb-8 rounded-xl border border-gray-200 bg-gray-50 p-4">
        ...
        <ol className="grid gap-1.5 sm:grid-cols-2">
          {GUIDE.map((s) => (
            <li key={s.id}>
              <a href={`#${s.id}`} className="flex items-center gap-2.5 rounded px-1.5 py-1 text-sm text-gray-700 hover:bg-white hover:text-clippa-red">
      ...
      <footer className="mt-12 border-t border-gray-200 pt-4 text-center text-xs text-gray-400">
        Clippa Sales Rep Router · Guide version {GUIDE_VERSION} · Powered by OuterJoin
      </footer>
    </div>
  );
}
