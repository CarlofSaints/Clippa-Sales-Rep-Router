 ...
 *
 * This exists as a separate route from PUT /api/reps precisely so that it can be
 * narrow: it writes three fields, on one record, chosen by the server. The rep
 * record id is never read from the request body — a body-supplied id is exactly
 * how `/api/auth/change-password` became an account-takeover path.
 *
 * Capturing the device's coordinates is the point of the whole feature. All 17
 * reps have no home address at all today, and an informal one typed in later
 * ("Stand no 644, next to the shop") geocodes to a suburb centroid with no error
...
/**
 * Body: { homeAddress } to type an address, and/or { lat, lng } captured from the
 * device. Device coordinates are authoritative and are stored as given — they are
 * a measurement, not a guess, so they skip the confidence gate that exists to
 * catch bad geocodes.
 */
  ...
  try {
    ...
    if (!rep) {
      return NextResponse.json(
        {
          error:
            "This login isn't linked to a rep record, so there is no profile to update. Ask your administrator.",
        },
        { status: 404 }
      );
    }
    ...
    const addressChanged = addressGiven && trimmedAddress !== (reps[idx].homeAddress || "").trim();
    ...
    if (hasDeviceFix) {
      ...
      // A changed address invalidates coordinates derived from the old one — the
      // same rule PUT /api/reps follows. Leaving them would anchor the rep's week
      // on where they used to live, silently and plausibly.
      reps[idx].homeGpsLat = "";
      reps[idx].homeGpsLng = "";
 ...
 * out-of-range numbers, and the (0,0) placeholder in the Gulf of Guinea that has
