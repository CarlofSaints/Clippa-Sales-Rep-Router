 ...
 * whole picture: it creates stores, creates channels, creates reps, and on
 * every row it touches it writes BOTH `existing.repCode = repCode` AND
 * `existing.monthlySales = sales` unconditionally. Send it a file that carries
 * only store details and it unassigns the rep and zeroes the monthly average on
 * every store in the sheet.
 *
 * This route reads the store-owned location fields and nothing else. It never
 * reads a rep code, never touches monthlySales, never creates a rep, and never
 * creates a store. Its whole reason to exist is that someone can export the
 * grid, fix a column of GPS coordinates in Excel, and send the file straight
 * back without needing the personnel or sales columns to be correct — or
 * present at all. 1 844 of the stores here have no coordinates, and the grid's
 * inline editor fixes them one at a time.
 */
