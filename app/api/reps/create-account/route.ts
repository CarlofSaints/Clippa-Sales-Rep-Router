...
// bcrypt at cost 10 plus a paced email per rep. The cap keeps a batch inside the
// function's time budget and inside Resend's send rate.
...
export interface CreateAccountOutcome {
   ...
   * created_no_email — account made, the mail failed; the password comes back so it can be shared by hand.
   * exists — a login already uses this email. Nothing was changed.
   * skipped — no usable email address on the rep, so there is nowhere to send a login.
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Every created user is pushed onto this one array and saved ONCE at the end.
    // Saving inside the loop would be a read-modify-write per rep against a single
    // blob, which is how a concurrent write silently loses accounts.
    ...
    for (const repId of repIds) {
      ...
      users.push(user);
      created.push({ user, password: tempPassword });
    }
    ...
    // Mail only AFTER the users are safely saved: a send failure must not cost us
    // the account, and an account we failed to save must never be emailed.
    for (let i = 0; i < created.length; i++) {
      ...
      outcomes.push({
        ...
        // Withheld on success: the password is in the rep's inbox, and putting it
        // on an admin's screen as well is a copy nobody needs.
        tempPassword: result.sent ? undefined : password,
      });
    }

    const madeCount = outcomes.filter(
      (o) => o.status === "created" || o.status === "created_no_email"
    ).length;
    if (madeCount > 0) {
      logActivity({
        ...
        summary: `Created ${madeCount} rep login${madeCount === 1 ? "" : "s"} (${
          outcomes.filter((o) => o.status === "created").length
        } emailed)`,
      });
    }
