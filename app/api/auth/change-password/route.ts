 ...
 * password with no session check at all — and it was reachable unauthenticated,
 ...
 * it. Both halves are fixed: public paths now match exactly, and the only
 * account this can touch is the caller's own.
 */
export async function POST(request: NextRequest) {
  try {
    // Whoever is signing in already holds a cookie here — POST /api/auth sets
    // one on valid credentials even when forcePasswordChange is true.
